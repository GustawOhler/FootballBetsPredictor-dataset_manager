...
from dataset_manager.common_funtions import create_match_infos
...
class DatasetWithSeparatedMatchesCreator(BaseDatasetCreator):
    def gather_data(self):
        ...
        for index, root_match in enumerate(self.matches_to_process.iterator()):
            ...
            dataset_row = NNDatasetRow(match_id=root_match.id,
                                       ...
                                       #TODO: poprawić!
                                       last_3_matches_between_teams=create_match_infos(last_3_matches_between_teams, root_home_team, root_match.date, 3))
    ...
    def save_dataset_to_csv(self):
        ...
        pd_dataset = pd.DataFrame(flatten(asdict(row), reducer='underscore', enumerate_types=(list,)) for row in self.dataset_objects)
